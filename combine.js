...
// //   '.db', '.sqlite', '.sqlite3',
// //   '.geojson', '.json', // GeoJSON and JSON data files
// //   '.woff', '.woff2', '.eot', // Font files
...
// //   'aboutHeroAnimation.json', // Lottie animations
// //   'chatbot.json',
// //   'ChatbotHelp.json',
// //   'cybersecurity.json',
// //   'detective.json',
...
// //   /\.tmp$/, // Temp files
// //   /\.cache$/, // Cache files
// //   /\.json$/, // All JSON files (config, data, animations)
...
// Binary file extensions & Build Artifacts
const EXCLUDED_EXTENSIONS = new Set([
  // Images & Media
  '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.svg',
  '.mp4', '.mp3', '.wav', '.avi', '.mov', '.webm',
  ...
  '.apk', '.zip', '.jar', '.dex', '.war', '.ear',
  '.exe', '.dll', '.so', '.o', '.a', '.dylib', '.bin',
  '.class', '.pyc', '.pyo', '.pyd', '.whl', '.egg',
  
  // Flutter/Mobile Build Artifacts
  '.dill', '.stamp', '.d', '.modulemap', '.plist', 
  '.xcconfig', '.xcuserstate',
  
  // Data/Logs
  '.data', '.index', '.prof', '.tfevents',
  '.json', '.geojson', '.lock', // Exclude JSON/Lock files to save space
  '.iml' // IDE files
]);

// Excluded directories (The Heavy Hitters)
const EXCLUDED_DIRECTORIES = new Set([
  // Standard Exclusions
  'node_modules', '.git', '.idea', '.vscode', '.gradle', 
  'build', 'dist', 'target', 'out', 'bin', 'obj',
  'coverage', '__pycache__', '.venv', 'venv', 'env',
  
  // FLUTTER SPECIFIC (Huge Savings)
  '.dart_tool',
  'flutter_build',  // Contains the massive hex-code folders
  'ephemeral',      // Linux/Windows build files
  '.symlinks',      // iOS/Mac plugin links
  'Pods',           // iOS external libraries (Source code not needed)
  '.cxx',           // Android C++ Build Cache (Massive)
  'generated',      // Generated code
  
  // SPRING BOOT / JAVA SPECIFIC
  'gradle',         // Wrapper folder (gradle-wrapper.jar etc)
  '.mvn',
  
  // ASSETS (Context Waste)
  'assets',
  'images',
  'fonts',
  'static',         // Spring Boot static assets
  
  // OPTIONAL: Exclude tests to focus purely on Logic
  // Comment out 'test' if you WANT the AI to see your unit tests
  'test', 
  'runner',         // Windows/Linux runner boilerplate
  'Runner.xcodeproj',
  'Runner.xcworkspace',
  'RunnerTests'
]);

// Excluded file names (Boilerplate & Configs)
const EXCLUDED_FILE_NAMES = new Set([
  OUTPUT_FILE,
  'combine.js', // Don't include this script
...
const COMMENT_SUPPORT = {
  // C-style comments (// and /* */)
  cStyle: ['.js', '.jsx', '.ts', '.tsx', '.java', '.dart', '.c', '.cpp', '.cc', '.h', '.hpp', '.cs', '.go', '.rs', '.swift', '.kt', '.scala', '.php'],
  // Python-style comments (#)
  python: ['.py', '.rb', '.sh', '.bash', '.yaml', '.yml', '.r', '.pl', '.conf', '.properties', '.gradle'],
  // HTML/XML comments (<!-- -->)
  html: ['.html', '.htm', '.xml', '.svg'],
  // CSS comments (/* */)
  css: ['.css', '.scss', '.sass', '.less'],
